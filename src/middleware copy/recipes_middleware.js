...
const {
  INVALID_RECIPE_NAME,
  DUPLICATED_RECIPE_NAME,
  INVALID_PRODUCED_AMOUNT,
  MIN_PROD_AMOUNT,
  INVALID_INGREDIENTS_ARRAY,
  INVALID_TYPE_MEASURE,
  INVALID_ID,
  DUPLICATED_INGREDIENT_NAME,
  INVALID_ARRAY_CONTENT,
} = require("../models/utils/Recipe-ErrorMSGs");
const { MEASURES_SHORT } = require("../models/utils/constants");
const {
  recipesPostController,
} = require("../controllers/recipe/recipe-post_controller");
const {
  recipesGetController,
  recipesGetByIdController,
} = require("../controllers/recipe/recipe-get_controller");
const {
  recipesDeleteController,
} = require("../controllers/recipe/recipe-delete_controller");
const {
  recipesPatchController,
} = require("../controllers/recipe/recipe-patch_controller");
const {
...
} = require("../controllers/Utils/aux_controller");
...
const processRecipePost = async (req, res) => {
  try {
    ...
    const { name, details, produced_amount, type_measure, ingredArray } =
      req.body;
    //! Validar si todos los ingredientes son del MISMO store
    await validateRecipePost(
      name,
      details,
      produced_amount,
      type_measure,
    ...
    const result = await recipesPostController(
      name,
      details,
      produced_amount,
      type_measure,
...
const processRecipePostRebuild = async (
  id,
  name,
  details,
  produced_amount,
  ...
  if (!isItAnExistingModelByID(id, store_id, Recipe))
  ...
  await recipesPatchController(
    oldRecipe.id,
    oldRecipe.name + actualDate,
    oldRecipe.details,
    oldRecipe.type_measure,
    store_id
  );
  const result = await recipesPostController(
    name,
...
const processRecipePatch = async (req, res) => {
  try {
    ...
    if (
      !(await isItAnExistingModelByID(id, store_id, Recipe)) ||
      (await isItAnExistingModelByName(name, store_id, Ingredient))
    )
      throw Error(`${INVALID_ID}${id}`);
...
const validateRecipePost = async (
  name,
  details,
  produced_amount,
  type_measure,
  ...
  if (await isItAnExistingModelByName(name, store_id, Ingredient))
    throw Error(`${DUPLICATED_RECIPE_NAME}${name}`);
  if (await isItAnExistingModelByName(name, store_id, Recipe))
    throw Error(`${DUPLICATED_INGREDIENT_NAME}${name}`);
  ...
  if (!(await validateArraySameStore(ingredArray, store_id, Recipe)))
    throw Error(INVALID_ARRAY_CONTENT);
  if (!MEASURES_SHORT.includes(type_measure))
    throw Error(`${type_measure}${INVALID_TYPE_MEASURE}`);
  return result;
};
