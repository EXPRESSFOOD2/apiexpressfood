...
} = require("../controllers/menuItem/menuItem-post_controller");
const {
  menuItemsGetController,
  menuItemsGetByIdController,
  menuItemsGetRecommendedController,
} = require("../controllers/menuItem/menuItem-get_controller");
const {
  menuItemsDeleteController,
} = require("../controllers/menuItem/menuItem-delete_controller");
const {
  menuItemsPatchController,
} = require("../controllers/menuItem/menuItem-patch_controler");
const {
  ERROR_NAME,
  INVALID_DECRIPTION,
  ERROR_PRICE,
  ...
  ERROR_NOT_FOUND,
  INVALID_ID,
  DUPLICATED_MENU_NAME,
} = require("../models/utils/MenuItem-ErrorMSGs");
const {
...
const processMenuPost = async (req, res) => {
  try {
    ...
    if (!(await validateToken(user_id, token)))
      throw Error("Token is invalid or expired, Please log in again.");
    ...
    const result = await menuItemsPostController(
      name,
      description,
      price,
      recomend_first,
...
const validateMenuItem = async (
  ...
  if (!description || !description.trim().length)
    throw Error(INVALID_DECRIPTION);
  ...
  if (!(await validateArraySameStore(ingredArray, store_id, Ingredient)))
...
const processMenuPatch = async (req, res) => {
  try {
    ...
    if (!(await isItAnExistingModelByID(id, store_id, MenuItem)))
      throw Error(ERROR_NOT_FOUND);

    const result = await menuItemsPatchController(
      id,
      name,
      description,
      price,
