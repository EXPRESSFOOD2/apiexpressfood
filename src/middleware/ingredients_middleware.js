...
} = require("../controllers/ingredient/ingredient-delete_controller");
...
const processIngredientDelete = async (req, res) => {
  try {
    ...
    if (!(await isItAnExistingModelByID(id, store_id, Ingredient)))
...
const processIngredientPatch = async (req, res) => {
  try {
    ...
    if (!(await isItAnExistingModelByID(id, store_id, Ingredient)))
      throw Error(`${INVALID_ID}${id}`);
    const result = await ingredientsPatchController(
      id,
      name,
      type_measure,
      store_id
...
const validateIngredient = async (
  ...
  if (!MEASURES_SHORT.includes(type_measure))
    throw Error(`${type_measure}${INVALID_MEASURE_TYPE}`);
};
