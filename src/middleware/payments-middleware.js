...
const paymentsMiddleware = async (req, res) => {
  try {
    ...
    if (!products.length)
      throw new Error("Request body must contain at least one product");

    if (!validateArraySameStore(products, store_id, MenuItem))
      throw new Error("Products belongs to diferent Stores!");

    products.forEach((product) => {
      if (typeof product !== "object")
        throw new Error("Request body must be an objects Array");
      if (!product.hasOwnProperty("id") || !product.hasOwnProperty("quantity"))
        throw new Error(
          "Each product must have a id property and quantity property"
        );
      if (
      ...
      if (product.quantity < 1)
        throw new Error("Quantity must be bigger than 1");
    });

    const result = await paymentsControllerPost(
      products,
      client_data,
      store_id
    );
