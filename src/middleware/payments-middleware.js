const paymentsMiddleware = (req, res, next) => {
  ...
  if (!Array.isArray(products)) return res.status(400).send( "Request must be an array");
  if (!products.length) return  res.status(400).send("Array must contain at least one product");
  products.forEach((product) => {
    if (typeof product !== "object") return  res.status(400).send("Request must be an objects Array");
    if (
