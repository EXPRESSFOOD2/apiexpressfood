...
const {
  tagsPostController,
} = require("../controllers/tag/tag-post_controller");
const {
  tagsPatchController,
} = require("../controllers/tag/tag-patch_controller");
...
} = require("../controllers/tag/tag-delete_controller");
const {
  getStoreIdByUserId,
...
const {
  ...
  DUPLICATED_TAG_NAME,
  INVALID_TAG_NAME,
} = require("../models/utils/Tag-ErrorMSGs");
const {
  tagsApplyController,
} = require("../controllers/tag/tag-apply_controller");
...
const processTagPatch = async (req, res) => {
  try {
    ...
    if (!(await isItAnExistingModelByID(id, store_id, Tag)))
...
const validateTagPost = async (name, store_id) => {
  ...
  if (await isItAnExistingModelByName(name, store_id, Tag))
    throw Error(`${DUPLICATED_TAG_NAME}${name}`);
  return true;
};
